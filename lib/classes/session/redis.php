...
class redis extends handler implements SessionHandlerInterface {
    ...
    /** @var int $connectiontimeout The number of seconds to wait for a connection response from the Redis server. */
    protected int $connectiontimeout = 3;

    /** @var int $readtimeout The number of seconds to wait for a read from the Redis server. */
    ...
    public function __construct() {
        ...
        if (isset($CFG->session_redis_read_timeout)) {
            $this->readtimeout = (int)$CFG->session_redis_read_timeout;
        }
