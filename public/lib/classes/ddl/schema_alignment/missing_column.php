<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

namespace core\ddl\schema_alignment;

/**
 * Class responsible for checking and fixing missing columns.
 *
 * @package    core
 * @copyright  2025 Benjamin Walker <benjaminwalker@catalyst-au.net>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class missing_column extends schema_issue {
    /** @var string The reference key for this type of schema issue */
    public const KEY = 'missingcolumns';

    /**
     * Constructor.
     *
     * @param string $message Description of the error.
     * @param \xmldb_table $table Table with schema issues.
     * @param \xmldb_field $field Column with schema issues.
     */
    public function __construct(
        /** @var string Description of the error. */
        public string $message,
        /** @var \xmldb_table Table with schema issues. */
        public \xmldb_table $table,
        /** @var \xmldb_field Column with schema issues. */
        public \xmldb_field $field,
    ) {
        $this->status = \core\check\result::ERROR;
        $this->safety = $this->can_add_column_safely() ? self::SAFE : self::UNFIXABLE;
    }

    /**
     * Attempts to create the missing column from the schema.
     *
     * @return bool True if the issue was resolved, false otherwise.
     */
    public function resolve_issue(): bool {
        if (!$this->get_dbman()->field_exists($this->table, $this->field)) {
            $this->print_cli('Creating missing column');
            $this->get_dbman()->add_field($this->table, $this->field);
            return true;
        }
        return false;
    }

    /**
     * Checks whether a field can be added to the table safely
     *
     * @return bool Whether the field can be added safely
     */
    protected function can_add_column_safely(): bool {
        global $DB;

        // Adding a column can be risky when the column doesn't allow nulls and there is no default.
        if (!$this->field->getNotNull() || $this->get_dbman()->generator->get_default($this->field) !== null) {
            return true;
        }

        // A column can still be added safely if there are no records. This should be quick to check.
        $records = $DB->record_exists($this->table->getName(), []);
        if ($records) {
            $this->message .= ", it cannot be added as the column has no default and doesn't allow nulls";
        }

        return !$records;
    }
}
