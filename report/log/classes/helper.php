...
class helper {
    ...
    public static function get_context_fallback(\core\event\base|\stdClass $event): string {
        ...
        $contexttype = \context_helper::get_level_name($event->contextlevel);
        ...
        if ($event->contextlevel == CONTEXT_MODULE || $event->contextlevel == CONTEXT_BLOCK) {
            ...
            // Get the pluginname if it exists (i.e. is still still installed).
            if (get_string_manager()->string_exists('pluginname', $component)) {
                $contexttype = get_string('pluginname', $component);
            } else {
                // Otherwise just remove the prefix and make the component human readable.
                $modulename = preg_replace('/^(mod_|block_)/', '', $component);
                $contexttype = ucfirst(str_replace('_', ' ', $modulename));
            }
        }

        // Can't access context get_url methods, so don't bother showing a url.
        return "$contexttype: $instanceid";
    }
}
